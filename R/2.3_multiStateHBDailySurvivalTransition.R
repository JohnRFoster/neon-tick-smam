...
model.code <- nimbleCode({
  ...
  for(s in 1:n.site){
    ...
    psi.pa.site[s] ~ dnorm(0, tau = tau.psi.pa.site)
    psi.ap.site[s] ~ dnorm(0, tau = tau.psi.ap.site)
    
    # survival and transition parameters estimated daily
    for(day in 1:n.days){
      logit(Sa.daily[s,day]) <- phi.a + phi.p.site[s]
      logit(Sp.daily[s,day]) <- phi.p + phi.a.site[s]
      logit(Fap.daily[s,day]) <- psi.ap + psi.pa.site[s]
      logit(Fpa.daily[s,day]) <- psi.pa + psi.ap.site[s]
    }
